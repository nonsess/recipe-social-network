...
class RecipeSearchRepositoryProtocol(Protocol):
    ...
    async def get_user_search_history(self, user_id: int, limit: int = 10) -> list[SearchQuery]: ...

    async def get_anonymous_search_history(self, anonymous_user_id: int, limit: int = 10) -> list[SearchQuery]: ...
